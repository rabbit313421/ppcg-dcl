...
HAVE_OPENCL=yes
HAVE_OPENMP=yes
srcdir="/home/jie/Documents/ppcg-dcl"
bindir="/home/jie/Documents/ppcg-dcl"
if [ $keep = "yes" ]; then
	OUTDIR="out.$VERSION"
...
run_tests () {
	...
	for i in `cat $DIR/utilities/benchmark_list`; do
		...
		dir=`dirname $i`
		if [ $verbose = "yes" ]; then
			echo ${bindir}/ppcg$EXEEXT -I $DIR/$dir $DIR/$i \
				$CPPFLAGS -o $source_opt $ppcg_options
		fi
		${bindir}/ppcg$EXEEXT -I $DIR/$dir $DIR/$i $CPPFLAGS \
			-o $source_opt $ppcg_options || exit

		kernel_opt="${OUTDIR}/$name.${ext}_kernel.c"
		if [ ! -f "$kernel_opt" ]; then
			kernel_opt=
		fi
		...
		if [ $verbose = "yes" ]; then
			echo $CC -I $DIR/$dir $CPPFLAGS $source_opt $kernel_opt \
				-o $prog_opt $DIR/utilities/polybench.c \
				$CFLAGS $cc_options
		fi
		$CC -I $DIR/$dir $CPPFLAGS $source_opt $kernel_opt -o $prog_opt \

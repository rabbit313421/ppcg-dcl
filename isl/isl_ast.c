...
//added by Jie Zhao
/* Check whether the body of an if guard which was converted from a 
 * for node has a break node. If so, check the id of its enclosing
 * node.
 */
int enclosing_body_has_break(__isl_keep isl_ast_node *node)
{
	...
	if(node->type == isl_ast_node_for){
		...
		if(body->type == isl_ast_node_block){
			int n = isl_ast_node_list_n_ast_node(body->u.b.children);
			for(int i = 0; i < n; i++){
				child_body = isl_ast_node_list_get_ast_node(body->u.b.children, i);
				if(isl_ast_node_get_type(child_body) == isl_ast_node_user){
					isl_ast_expr *expr, *arg;
					expr = isl_ast_node_user_get_expr(child_body);
					arg = isl_ast_expr_get_op_arg(expr, 0);
					id = isl_ast_expr_get_id(arg);
					if (!strcmp(isl_id_get_name(id), "break")){
						struct ppcg_kernel_stmt *stmt;
						id = isl_ast_node_get_annotation(child_body);
						stmt = isl_id_get_user(id);
						if(stmt->u.b.is_dcl)
							n_label = stmt->u.b.n_label;
						return n_label;
					}
				}
			}
	...
	__isl_keep isl_ast_print_options *options)
{
	...
	//added by Jie Zhao
	int label_id = enclosing_body_has_break(node);
	...
	__isl_keep isl_ast_print_options *options)
{
	...
	for (i = 0; i < list->n; ++i){
		...
		//Added by Jie Zhao
		int label_id = enclosing_body_has_break(list->p[i]);
		if(label_id != -1){		
			p = isl_printer_start_line(p);
			p = isl_printer_print_str(p, "label");
			p = isl_printer_print_int(p, label_id);

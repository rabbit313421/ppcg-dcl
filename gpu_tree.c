...
//added by Jie Zhao
/* Move up the tree underneath the "dynamic_counted_loops" mark until
 * the "dynamic_counted_loops" mark is reached.
 */
...
//added by Jie Zhao
/* Move down the branch between "kernel" and "dynamic_counted_loops" until
 * the "dynamic_counted_loops" mark is reached, where the branch containing
...
//added by Jie Zhao
/* Is "id" the identifier of a break statement inside "kernel"?
 * That is, does its name start with "break" and does it point to "kernel"?
 */
int gpu_tree_id_is_break(__isl_keep isl_id *id, struct ppcg_kernel *kernel)
{
	const char *name;
...
//added by Jie Zhao
/* Does "domain" consist of a single set with a tuple identifier
 * corresponding to a break for "kernel"?
 */
static int domain_is_break(__isl_keep isl_union_set *domain,
	struct ppcg_kernel *kernel)
{
	int is_break;
	isl_id *id;
	...
	set = isl_set_from_union_set(isl_union_set_copy(domain));
	id = isl_set_get_tuple_id(set);
	is_break = gpu_tree_id_is_break(id, kernel);
	isl_id_free(id);
	isl_set_free(set);
...
//added by Jie Zhao
/* Does "node" point to a filter selecting a break statement
 * for "kernel"?
 */
static int node_is_break_filter(__isl_keep isl_schedule_node *node,
	struct ppcg_kernel *kernel)
{
	int is_break;
	enum isl_schedule_node_type type;
	...
	if (type != isl_schedule_node_filter)
		return 0;
	domain = isl_schedule_node_filter_get_filter(node);
	is_break = domain_is_break(domain, kernel);
	isl_union_set_free(domain);
...
//added by Jie Zhao
/* Is "node" part of a sequence with a subsequent break statement
 * for "kernel"?
 * That is, is the parent of "node" a filter such that there is
 * a subsequent filter that picks out exactly such a break statement?
 */
static int has_following_break(__isl_keep isl_schedule_node *node,
	struct ppcg_kernel *kernel)
{
	int found = 0;
	...
	while (!found && isl_schedule_node_has_next_sibling(node)) {
		...
		if (!node)
			break;
		found = node_is_break_filter(node, kernel);
	}
	if (!node)
		found = -1;
	isl_schedule_node_free(node);
 ...
 * That is, is the parent of "node" a filter such that there is
 * a subsequent filter that picks out exactly such a break statement?
 * If so, find this statement.
 */
static __isl_give isl_schedule_node *find_following_break(
	...
	__isl_take isl_schedule_node *node, struct ppcg_kernel *kernel)
{
	...
	has_break = has_following_break(node, kernel);

	if(has_break){
		node = find_following_break(node, kernel);
		node = isl_schedule_node_child(node, 0);
...
//added by Jie Zhao
/* Insert (or extend) an extension on top of "node" that puts
 * a break node for "kernel" after "node".
 * Return a pointer to the original node in the updated schedule tree.
 */
static __isl_give isl_schedule_node *insert_break_after(
	__isl_take isl_schedule_node *node, struct ppcg_kernel *kernel)
{
	isl_union_set *domain;
	isl_schedule_node *graft;
	...
	domain = create_break_domain(kernel);
	graft = isl_schedule_node_from_domain(domain);
	node = isl_schedule_node_graft_after(node, graft);
